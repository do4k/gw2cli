...
public static class ItemCommands
{
    public static Command Build(ConfigService config, GW2ApiService api, Option<string?> apiKeyOption)
    {
        ...
        items.AddCommand(BuildGet(api, apiKeyOption));
        return items;
    }
        ...
        cmd.SetHandler(async (InvocationContext ctx) =>
        {
            ...
            await Helpers.RunAsync(async () =>
            {
                ...
                if (!string.IsNullOrEmpty(item.Description))
                    Row("Description", Markup.Escape(item.Description));

                if (item.Flags.Count > 0)
                    Row("Flags", string.Join(", ", item.Flags));

                if (item.Restrictions.Count > 0)
                    Row("Restrictions", string.Join(", ", item.Restrictions));
                ...
                if (!string.IsNullOrEmpty(item.ChatLink))
                    Row("Chat Link", Markup.Escape(item.ChatLink));
